...
- `vite/` : Viteを使用したフロントエンドのビルド設定やシミュレーションファイルを格納するフォルダー
    ...
    - `ts/` : 共通のTypeScriptファイルを格納するフォルダー
...
- p5.jsの記述スタイルはインスタンスモード（`new p5(sketch)`）に統一します。新規シミュレーションは`npm run new:simulation`（`scripts/new-simulation.js`）が生成するひな形（ESモジュール + インスタンスモード）に従い、グローバルモード（`function setup()`の直接定義）では実装しないでください。詳細は[シミュレーション実装方法](docs/docs/simulation/index.md#実装パターン)を参照してください。
