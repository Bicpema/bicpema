...
`vite/simulations/<slug>/ts/logic.ts` 等のロジックをユニットテスト対象にするには、`state` の読み書きや `p.` 呼び出しを関数内で直接行わず、次のように入出力が明確な純粋関数として切り出します。
...
対象は `tsconfig.json` の `include` に列挙されたファイルです。シミュレーションのロジックに型チェックを追加する場合は、対象ディレクトリを `include` に追加してください。`strict: true` が有効化されているため、新規・既存を問わずすべての対象ファイルで厳格な型チェックが適用されます。
