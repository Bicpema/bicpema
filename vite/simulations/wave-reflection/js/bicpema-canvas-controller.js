/**
 * 画面サイズの管理と波のクラス定義を行うクラス。
 */
export class BicpemaCanvasController {
  /**
   * {@link BicpemaCanvasController}クラスのコンストラクタ。
   * @param {*} f 画面サイズを固定するかどうかのフラグ。trueの場合、16:9のアスペクト比を維持して画面サイズを調整する。falseの場合、ウィンドウ全体を使用する。
   * @param {*} i 3D描画を使用するかどうかのフラグ。trueの場合、WEBGLモードでキャンバスを作成する。falseの場合、通常の2Dモードでキャンバスを作成する。
   * @param {*} w_r キャンバスの幅の比率。画面全体を使用する場合は1.0、画面の一部を使用する場合は0.5など。
   * @param {*} h_r キャンバスの高さの比率。画面全体を使用する場合は1.0、画面の一部を使用する場合は0.5など。
   */
