...
export function onHeightChange() {
  ...
  if (!state.ball!.isMoving) {
    state.ball!.reset(
      newHeight,
      parseFloat(String(state.initialVelocityInput!.value()))
    );
    state.graph!.reset();
  }
}
...
export function onInitialVelocityChange() {
  let newVelocity = parseFloat(String(state.initialVelocityInput!.value()));
...
export function onReset() {
  ...
  const newInitialVelocity = parseFloat(
