...
function setup() {
    ...
    for (let i = 0; i < hm; i++) {
        balls[i] = new Ball(
            dataArray.getNum(i, 3),
            asin(100 / dataArray.getNum(i, 3))
        );
    }
    gravity = 9.8;
    count = 0;
...
function mousePressed() {
    if (
        width - (4 * stop_watch.width) / 5 < mouseX &&
        mouseX < width - (4 * stop_watch.width) / 5 + start_button.width &&
        height - stop_watch.height + (11 * stop_watch.height) / 20 < mouseY &&
        mouseY <
            height -
                stop_watch.height +
                (11 * stop_watch.height) / 20 +
                start_button.height
    ) {
    ...
    if (
        width - (4 * stop_watch.width) / 5 + start_button.width < mouseX &&
        mouseX < width - (4 * stop_watch.width) / 5 + 2 * start_button.width &&
        height - stop_watch.height + (11 * stop_watch.height) / 20 < mouseY &&
        mouseY <
            height -
                stop_watch.height +
...
class Ball {
    ...
    display() {
        ...
        image(
            weight,
            this.posx - weight.width / 2,
            this.posy - weight.height / 2
        );
    }
}
