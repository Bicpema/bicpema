...
function fullScreen() {
    ...
    let canvas = createCanvas(
        p5Canvas.clientWidth,
        p5Canvas.clientHeight,
        WEBGL
    );
...
function colabNum1_filledimage() {
    if (colabNum == 1) {
        ...
        if (!Cluster1isDead) {
            ...
            (hAfter1,
                sAfter1,
                (vAfter1 = rgbToHsv1(rAfter1, gAfter1, bAfter1)));
            Cluster1isDead = true;
        }
        // 位置情報の獲得
    ...
    if (colabNum >= 2) {
        ...
        if (clusterCount == 1) {
            ...
            for (let n = 0; n < colabNum; n++) {
                // 0 から colabNum-1 に修正
                let numInputValue2 = parseInt(
                    select("#numInput-" + (n + 1)).value()
                ); // 数値型に変換
        ...
        for (let i = 380; i <= 750; i++) {
            ...
            if (colabNum >= 2) {
                for (let n = 2; n <= colabNum; n++) {
                    ...
                    let b = radians(
                        targetAngle.value() - referenceAngle.value()
                    );
                    E_2 = math.multiply(
                        r_theta(b),
            ...
            osArr[i - 380] =
                relativeStrength * osArrOrigin[i - 380] * R_all[i - 380];
                ...
                for (let j = 1; j < colabNum; j++) {
                    ...
                    if (bit[j] == 0) {
                        E_2 = math.multiply(
                            ...
                            math.multiply(
                                cello,
                                math.multiply(mai_r_theta(b), E_2)
                            )
                        );
                    } else {
                        E_2 = E_2;
    ...
    for (let iter = 0; iter < iterations; iter++) {
        ...
        for (let y = 0; y < img2.height; y++) {
            for (let x = 0; x < img2.width; x++) {
                ...
                let maxCount = neighborLabels.filter(
                    (l) => l === currentLabel
                ).length;
    ...
    let mainData = {
        ...
        datasets: [
            ...
            },
            {
                ...
                borderColor:
                    "rgba(" + rBefore + "," + gBefore + "," + bBefore + ",1)",
                pointRadius: 0,
                fill: "start",
                showLine: true,
            },
