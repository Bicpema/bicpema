...
let ls_xArrAfter = [], ls_yArrAfter = [], ls_zArrAfter = []; //明るさ調節の為の光源の強さに相当
...
let tape_angle_get;
let radius; //セロハンのサイズ(高さ) 2024.3.17までは111
let precolabNum;
let rAftera;
let gAftera;
let bAftera;
...
//クラスター分類する際に新たに追加した変数(2025.3.18)
let clusters = 4;
let clusterColors = [];
let labels = [];
let edgePixels = [];
...
// 外部ファイルの読み込み
function preload() {
    cmfTable = loadTable("https://firebasestorage.googleapis.com/v0/b/bicpema.firebasestorage.app/o/public%2Fassets%2Fcsv%2Fcommon%2Fcmf.csv?alt=media&token=df4cb716-5da8-4640-822e-5107acbdb916", "csv", "header") // 等色関数のデータ
    osTable = loadTable("https://firebasestorage.googleapis.com/v0/b/bicpema.firebasestorage.app/o/public%2Fassets%2Fcsv%2Fcommon%2Fos_PC2_new_6.18.csv?alt=media&token=0ba4f938-5669-456b-81dc-e4c62c66ce46", "csv", "header") // 偏光板を一枚通したときの波長毎の強度分布 PC-最新
    dTableOPP = loadTable("https://firebasestorage.googleapis.com/v0/b/bicpema.firebasestorage.app/o/public%2Fassets%2Fcsv%2Fcommon%2Fdata_d_100_film3.csv?alt=media&token=68edd450-dd93-4b8b-851f-28c1ffe14999.csv", "csv", "header")//光路差の分散特性(380nmで100に規格化)
    dTable = loadTable("https://firebasestorage.googleapis.com/v0/b/bicpema.firebasestorage.app/o/public%2Fassets%2Fcsv%2Fcommon%2Fdata_d_100.csv?alt=media&token=eaf5a4d5-ab04-42fd-8245-eb4896a5eaf5", "csv", "header")
    rTable = loadTable("https://firebasestorage.googleapis.com/v0/b/bicpema.firebasestorage.app/o/public%2Fassets%2Fcsv%2Fcommon%2FR.csv?alt=media&token=203b2f68-a0c0-42c2-af5e-df5c240ea27d", "csv", "header")//偏光板2枚目による強度補正分のdata
    img = loadImage("https://firebasestorage.googleapis.com/v0/b/bicpema.firebasestorage.app/o/public%2Fassets%2Fimg%2F2025%3DDGI%3Dcellophane-color2_ELK%2Fwhite.png?alt=media&token=038ee120-ec5e-4440-8130-3b764f11d25e");
    img2 = loadImage("https://firebasestorage.googleapis.com/v0/b/bicpema.firebasestorage.app/o/public%2Fassets%2Fimg%2F2025%3DDGI%3Dcellophane-color2_ELK%2FR.jpg?alt=media&token=9e82b742-fe5b-4332-af54-5796f92bd9ba");
}
...
function initValue() {
    ...
    //分割計算する際に必要な配列
    for (let n = 1; n <= 15; n++) {
        last_otherCellophaneNums[n - 1] = 1; // last_otherCellophaneNums[n-2]=1; 
        last_targetAngles[n - 1] = 1; //   last_targetAngles[n-2]=1
        last_opt[n - 1] = 1;
    }
    // クラスター分類の際のsetup
    colorMode(RGB, 255, 255, 255);
    copyimg = img2.get(); // 元画像をコピー
    clusterCount = 0;
    clusterCount1 = 0;
...
function colabNum2_normal() {
    ...
    if (colabNum >= 2) {
        ...
        let check = 0;
        for (let n = 1; n <= colabNum; n++) { //1でなく2では..?
            let numInputValue = parseInt(select("#numInput-" + n).value()); // 数値型に変換
            let rotateInputValue = parseFloat(select("#rotateInput-" + n).value()); // 数値型に変換
            let optInputValue = parseFloat(select("#opdInput-" + n).value()); // 数値型に変換
            if (numInputValue !== last_otherCellophaneNums[n - 2]) {
        ...
        if (!BisDead) {
            ...
            if (end == Math.pow(2, colabNum)) { BisDead = true }
        }
...
function colabNum2_filledimage() {
    ...
    if (colabNum >= 2) {
        ...
        if (clusterCount == 1) {
            ...
            for (let n = 0; n < colabNum; n++) {  // 0 から colabNum-1 に修正
...
function afterColorCalculate() {
    ...
    if (colabNum >= 1) {
        ...
        for (let i = 380; i <= 750; i++) {
            ...
            let l = i
            let delta = dArr[i - 380] * firstCellophaneNum.value() * 2 * firstopdInput.value() * PI / l / 100
            ...
            if (colabNum >= 2) {
                for (let n = 2; n <= colabNum; n++) {
                    ...
                    let delta = dArr[i - 380] * otherCellophaneNum.value() * 2 * otheropdInput.value() * PI / l / 100
                    let cello = [[1, 0], [0, math.exp(math.complex(0, -delta))]]
...
function afterColorCalculates(binaryString) {
    ...
    if (numStart !== 0) {
        ...
        for (let i = 380; i <= 750; i++) {
            let l = i
            let firstopdInput = select("#opdInput-1")// セロハン1組目の光路差
            let delta = dArr[i - 380] * firstCellophaneNum.value() * 2 * firstopdInput.value() * PI / l / 100//2024.6.22 firstCellophaneの値をvalueで数値化しないとだめだった!
            let cello = [[1, 0], [0, math.exp(math.complex(0, -delta))]]
            E_2 = math.multiply(cello, E_1)
...
function getrectPoint(tape_angle) {
    push();
    translate(-100, -100);
    let sinValues = [sin(angle_1 + tape_angle - PI / 2), sin(angle_2 + tape_angle - PI / 2), sin(angle_3 + tape_angle - PI / 2), sin(angle_4 + tape_angle - PI / 2)];
    let cosValues = [cos(angle_1 + tape_angle - PI / 2), cos(angle_2 + tape_angle - PI / 2), cos(angle_3 + tape_angle - PI / 2), cos(angle_4 + tape_angle - PI / 2)];
        ...
        for (let x = 1; x < img2.width - 1; x++) {
            ...
            for (let dy = -1; dy <= 1; dy++) {
                for (let dx = -1; dx <= 1; dx++) {
                    ...
                    let intensity = (img2.pixels[nIndex] + img2.pixels[nIndex + 1] + img2.pixels[nIndex + 2]) / 3;
...
function kmeans(data, k) {
    ...
    let sortedClusters = Array.from({ length: k }, (_, i) => i).sort((a, b) => clusterSizes[b] - clusterSizes[a]);
...
function drawGraph2_1(x1, y1) {
    ...
    // 円のデータを作成（360分割）
    let circleArr = [];
    for (let theta = 0; theta < 2 * Math.PI; theta += Math.PI / 180) { // 1度刻み
