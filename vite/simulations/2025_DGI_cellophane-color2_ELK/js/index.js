...
const sketch = (p) => {
  ...
  p.setup = () => {
    const p5CanvasEl = document.getElementById("p5Canvas");
    const canvas = p.createCanvas(
      p5CanvasEl.clientWidth || p.windowWidth / 2,
      p5CanvasEl.clientHeight || 400,
      p.WEBGL
    );
    canvas.parent(p.select("#p5Canvas")).class("rounded border border-1");
    elCreate(p);
    initValue(p);
