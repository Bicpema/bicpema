        ...
        for (let i = 0; i < trNum; i++) {
            ...
            for (let j = 0; j < strataArr.length; j++) {
                if (strataKind == strataArr[j][2]) {
                    element.option(
                        strataArr[j][0] + "m-" + strataArr[j][1] + "m"
                    );
                }
            }
        }
...
function loadTestDataButtonFunction() {
    if (Object.keys(dataInputArr).length == 0) {
        ...
        let place_arr = [
            [
                35.660552, 35.668404, 35.660752, 35.666669, 35.654647,
                35.672779, 35.660607,
            ],
            [
                139.516632, 139.519548, 139.507364, 139.507854, 139.523045,
                139.508945, 139.538435,
            ],
        ];
        let test_data = {
...
function drawStrata(key, rotateTime, xMin, xMax, yMin, yMax, zMin, zMax) {
    ...
    for (let i = 0; i < layer.length; i++) {
        ...
        translate(
            x,
            map(z, zMin, zMax, 0, 500) +
                map(zLength, 0, zMax - zMin, 0, 500) / 2,
            y
        );
...
class DOM {
    constructor(n) {
        ...
        this.xInput = createInput(0, "number")
            ...
            .class("form-control");
        createDiv(
            "地点" + str(this.n) + "の名前、緯度、経度を入力してください。"
        )
            .parent(this.parentDiv)
            .class("form-text");
        // サブウィンドウ生成用のDOM
