...
import {
  CELLOPHANE_RADIUS,
  POLARIZER_SIZE,
  CELLOPHANE_THICKNESS,
  CROSSED_POLARIZER_OFFSET_DEG,
  GUIDE_LINE_Z_OFFSET,
  GUIDE_LINE_HALF_LENGTH,
  XYZ_TO_SRGB_MATRIX,
  WAVELENGTH_MIN,
  WAVELENGTH_MAX,
...
export function drawSimulation(p: p5) {
  ...
  state.radius = CELLOPHANE_RADIUS;
  prenormal(p);
  colabNum1_normal(p);
  colabNum2_normal(p);
...
function checked(p: p5) {
  // 基準線(0°)
  p.push();
  p.translate(0, 0, GUIDE_LINE_Z_OFFSET);
  p.stroke(255, 0, 0);
  p.line(0, GUIDE_LINE_HALF_LENGTH, 0, -GUIDE_LINE_HALF_LENGTH);
  p.pop();
...
function prenormal(p: p5) {
  ...
  // テープ描画における条件設定(幅)
  state.angle_1 = p.atan2(100, Number(state.slider!.value()));
  state.angle_2 = p.PI - p.atan2(100, Number(state.slider!.value()));
  state.angle_3 = p.PI + p.atan2(100, Number(state.slider!.value()));
  state.angle_4 = 2 * p.PI - p.atan2(100, Number(state.slider!.value()));
  ...
  // 偏光板の描画
  createPolarizer(p, POLARIZER_SIZE, 0, 0, 0, 0);
  ...
  if (state.polarizerSelect!.value() === "平行ニコル配置") {
    createPolarizer(
      ...
      0,
      0,
      -CELLOPHANE_THICKNESS * state.cellophaneNum,
      0
    );
  }
  ...
  if (state.colabNum >= 2) {
    if (state.count2 === 0) {
      for (let i = 0; i < state.img!.pixels.length; i += 4) {
        ...
        state.img!.pixels[i + 3] = INITIAL_PIXEL_COLOR[3]; //7.13までは80
      }
      state.img!.updatePixels();
      state.count2 = 1;
    }
    ...
    state.currentSlider = Number(state.slider!.value());
    let check = 0;
    ...
    if (!state.BisDead) {
      ...
      if (state.Bcount === 0) {
        // rgbを格納する配列の初期化
        state.rAftera = new Array(2 ** state.colabNum); //2024.6.17 colabNum修正 colabNum-1から
        state.gAftera = new Array(2 ** state.colabNum);
        state.bAftera = new Array(2 ** state.colabNum);
        state.Bcount += 1;
      }
      p.push();
      p.fill(255, 0, 0, 10);
      ...
      if (typeof state.Bsize === "undefined") {
        state.Bsize = BATCH_SIZE_INITIAL;
      }
      if (p.frameRate() < BATCH_FPS_THRESHOLD) {
        state.Bsize = p.max(BATCH_SIZE_MIN, state.Bsize! / 2); // フレームレートが低い場合、Bsizeを小さく
      } else {
        state.Bsize = p.min(
          BATCH_SIZE_MAX,
          state.Bsize! * BATCH_SIZE_GROWTH_FACTOR
        ); // フレームレートが高い場合、Bsizeを大きく
      }
...
function createCellophane(
  ...
  for (let i = 0; i < n; i++) {
    p.push();
    p.translate(-0, 0, -CELLOPHANE_THICKNESS * (i + a));
    p.box(
...
function toRGB(a: number) {
  if (a <= SRGB_GAMMA_THRESHOLD) {
    a *= SRGB_LINEAR_SCALE;
  } else {
    a = SRGB_GAMMA_SCALE * a ** SRGB_GAMMA_EXPONENT - SRGB_GAMMA_OFFSET;
  }
  // 0〜1にクリップ
  ...
  if (state.colabNum >= 1) {
    ...
    for (let i = WAVELENGTH_MIN; i <= WAVELENGTH_MAX; i++) {
      ...
      if (state.polarizerSelect!.value() === "平行ニコル配置") {
        ...
        c =
          p.radians(-Number(referenceAngle.value())) -
          p.radians(CROSSED_POLARIZER_OFFSET_DEG);
      }
...
function afterColorCalculates(p: p5, binaryString: string) {
  ...
  if (numStart !== 0) {
    ...
    state.rAfter2 = DEFAULT_GRAY;
    state.gAfter2 = DEFAULT_GRAY;
    state.bAfter2 = DEFAULT_GRAY;
  } else if (state.polarizerSelect!.value() === "直交ニコル配置") {
    state.rAfter2 = 0;
    state.gAfter2 = 0;
    state.bAfter2 = 0;
...
function drawTape_1(
  ...
  for (let i = 0; i < state.img!.pixels.length; i += 4) {
    if (checkA(i / 4)) {
      ...
      state.img!.pixels[i + 0] = DEFAULT_GRAY;
      state.img!.pixels[i + 1] = DEFAULT_GRAY;
      state.img!.pixels[i + 2] = DEFAULT_GRAY;
    } else if (state.polarizerSelect!.value() === "直交ニコル配置") {
