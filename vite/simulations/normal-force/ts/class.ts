...
function arrow(
  ...
  a: number,
  b: number,
  n: "gravity" | "vertical" | "horizontal" | "normal"
) {
  if (n === "gravity") {
    p.triangle(
      a,
    ...
    dashedLine(
      ...
      b +
        (state.materialWidth *
          ...
          2 +
        ARROW_LENGTH *
          p.sq(p.cos(p.radians(Number(state.slopeAngleButton!.value())))),
      1
    );
    dashedLine(
      p,
      ...
      b +
        (state.materialWidth *
          ...
          2 +
        ARROW_LENGTH *
          p.sq(p.sin(p.radians(Number(state.slopeAngleButton!.value())))),
      1
    );
    p.line(
      a +
  ...
  _draw(p: p5) {
    if (state.clickedCount === true) {
      const { dx, dy } = computeSlideDisplacement(
        Number(state.gravityButton!.value()),
        Number(state.slopeAngleButton!.value()),
        state.count
      );
      this.materialX += dx;
      this.materialY += dy;
