 ...
 */
export function drawTank(cx, cy, tankW, tankH, tankD, wallThickness) {
  const halfW = tankW / 2;
 ...
 */
export function drawWater(cx, waterSurfaceY, tankBottomY, tankW, tankD) {
  const halfW = tankW / 2;
 ...
 * @param {number} cx 水槽の中心X座標
 */
export function drawCylinder(cylinder, waterSurfaceY, tankW, tankD, cx) {
  const r = cylinder.r;
  const h = cylinder.h;
 ...
 * @param {number} waterSurfaceY 水面のY座標
 */
export function drawInfoText(cylinder, waterSurfaceY) {
