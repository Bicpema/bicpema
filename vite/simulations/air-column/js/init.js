...
export function setupControls(p) {
  ...
  mnPlusBtn.addEventListener('click', () => {
    if (state.type === 'closed') { state.m_n = Math.min(9, state.m_n + 2); }
    else { state.m_n = Math.min(9, state.m_n + 1); }
    updateDisplays();
    updateWaveLayer(p);
  });

  mnMinusBtn.addEventListener('click', () => {
    if (state.type === 'closed') { state.m_n = Math.max(1, state.m_n - 2); }
    else { state.m_n = Math.max(1, state.m_n - 1); }
    updateDisplays();
    updateWaveLayer(p);
  });
