...
function mousePressed() {
  ...
  if (
    dist(
      width - rotateRemocon.width + (9 * rotateRemocon.width) / 10,
      height - rotateRemocon.height + (3 * rotateRemocon.height) / 10,
      mouseX,
      mouseY
    ) <
      rotateRemocon.width / 20 &&
    lightRotateTheta < 90
  ) {
  ...
  if (
    dist(
      width - rotateRemocon.width + (9 * rotateRemocon.width) / 10,
      height - rotateRemocon.height + (7 * rotateRemocon.height) / 10,
      mouseX,
      mouseY
    ) <
      rotateRemocon.width / 20 &&
  ...
  if (
    dist(
      (9 * nRemocon.width) / 10,
      height / 2 - nRemocon.height + (3 * nRemocon.height) / 10,
      mouseX,
      mouseY
    ) <
    nRemocon.width / 20
  ...
  if (
    dist(
      (9 * nRemocon.width) / 10,
      height / 2 - nRemocon.height + (7 * nRemocon.height) / 10,
      mouseX,
      mouseY
    ) <
      nRemocon.width / 20 &&
  ...
  for (let i = 0; i < 4; i++) {
    if (
      width - ((4 - i) * width) / 8 < mouseX &&
      mouseX < width - ((4 - i - 1) * width) / 8 &&
      0 < mouseY &&
      mouseY < height / 20
    ) {
      if (i == 0) {
...
function animationOperation() {
  ...
  image(
    rotateRemocon,
    width - rotateRemocon.width,
    height - rotateRemocon.height
  );
  text(
    nf(abs(lightRotateTheta), 1, 1) + "'",
    width - rotateRemocon.width + (5 * rotateRemocon.width) / 12,
    height - rotateRemocon.height + rotateRemocon.height / 4,
    rotateRemocon.width / 3,
    rotateRemocon.height / 2
  );
  image(nRemocon, 0, height / 2 - nRemocon.height);
  text(
    nf(n2, 1, 1) + "'",
    (5 * nRemocon.width) / 12,
    height / 2 - nRemocon.height + nRemocon.height / 4,
    nRemocon.width / 3,
    nRemocon.height / 2
  );
...
function lineRays() {
  ...
  line(
    width / 2,
    height / 2,
    width / 2 - (height / 2 - height / 6) * sin(theta1),
    height / 2 + (height / 2 - height / 6) * cos(theta1)
  );
  if (-1 < boundary && boundary < 1) {
  ...
  line(
    width / 2,
    height / 2,
    width / 2 + width * sin(theta1),
    height / 2 + width * cos(theta1)
  );
  stroke(255, 0, 0, 255 * (1 - strength));
  line(
    width / 2,
    height / 2,
    width / 2 + width * sin(theta2),
    height / 2 - width * cos(theta2)
  );
}
