...
同じ物質で質量も等しい2つの物体の場合、熱平衡温度 \\( T \\) は以下のように求められます:
...
- \\( m_1, m_2 \\): 各物体の質量 [\\( \\text{g} \\)]
- \\( c_1, c_2 \\): 各物体の比熱 [\\( \\text{J/(g·K)} \\)]
- \\( T \\): 熱平衡温度 [\\( \\text{K} \\)]
...
$$
Q_1 = Q_2, \quad m_1 c_1 (T_1 - T) = m_2 c_2 (T - T_2)
$$
...
1. シミュレーションを開始すると、高温物体（373.0 \\( \\text{K} \\)）と低温物体（50.0 \\( \\text{K} \\)）が分離した状態で表示されます
...
6. グラフの横軸は接触してからの経過時間[\\( \\text{s} \\)]、縦軸は温度[\\( \\text{K} \\)]を表しています
