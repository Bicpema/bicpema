...
$$
\mathbf{v}_{\text{合成}} = \mathbf{v}_{\text{川}} + \mathbf{v}_{\text{船}}
$$

このシミュレーションでは以下のパラメータを調整できます:

- **川の速度** (\\( v\_{\text{川}} \\)): 川の流速（左向き固定、0〜10 \\( \\text{m/s} \\)）
- **船の速度** (\\( v\_{\text{船}} \\)): 船が水に対して進む速度（＋＝下流左向き、−＝上流右向き、−10〜+10 \\( \\text{m/s} \\)）

シミュレーションでは、赤色の矢印が川の速度 (\\( v\_{\text{川}} \\))、緑色の矢印が船の速度 (\\( v\_{\text{船}} \\))、青色の矢印が合成速度 (\\( v\_{\text{合}} \\)) を示します。

### 観察できる現象

- \\( v\_{\text{船}} > 0 \\)（下流方向）のとき: \\( v\_{\text{合}} = v\_{\text{川}} + v\_{\text{船}} \\)（合成速度は川よりも速い左向き）
- \\( v\_{\text{船}} = 0 \\) のとき: \\( v\_{\text{合}} = v\_{\text{川}} \\)（船は川に流されるのみ）
- \\( v\_{\text{船}} < 0 \\)（上流方向）のとき:
    - \\( |v\_{\text{船}}| < v\_{\text{川}} \\) なら船は左向きに進む（遅くなる）
    - \\( |v\_{\text{船}}| = v\_{\text{川}} \\) なら船は静止して見える（\\( v\_{\text{合}} = 0 \\)）
    - \\( |v\_{\text{船}}| > v\_{\text{川}} \\) なら船は右向きに進む（上流へ）
...
2. 「川の速度」スライダーで川の流速を設定します（0〜10 \\( \\text{m/s} \\)）
3. 「船の速度」スライダーで船が水に対して進む速度を設定します（−10〜+10 \\( \\text{m/s} \\)、＋＝下流・−＝上流）
...
- 船が上流へ向かう速度が川の流速と等しいとき（\\( |v\_{\text{船}}| = v\_{\text{川}} \\)）、合成速度が 0 になり、岸から見ると船は静止して見えます
